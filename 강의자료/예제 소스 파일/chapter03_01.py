...
list = [str1, str2] # 대괄호
dict = { 
    "name": "Machine Learning", # 키 : name , value : Machine Learning
    "version": 2.0 #
}
tuple = 3, 5, 7 # 두가지 선언법이 있음
tuple = (3, 5, 7) # 소괄호
set = {7, 8, 9} # 중괄호
'''
...
print(float(True))  # Bool -> 정수 #0.ㅇ
print(int(False))  # Bool -> 정수
print(float(False))  # Bool -> 정수
print(complex(3))  # 정수 -> 복소수
print(complex('3'))  # 문자 -> 복소수, 문자를 넣어도 제대로 출력이 된다
print(complex(False))  # Bool -> 복소수

# 수치 연산 함수
print(abs(-7)) # 절대값 반환
x, y = divmod(100, 8) # 100을 8로 나눈다 x는 몫 y는 나머지가 할당된다
print(x, y)
print(pow(5, 3)) # 제곱 5의 3승
